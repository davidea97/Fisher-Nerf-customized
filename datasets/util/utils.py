...
def get_sim_transform(agent_state):
    """ Get 4x4 transform from agent state (c2w) """
    x = -agent_state.position[2]
    y = -agent_state.position[0]
    height = agent_state.position[1]
    
    rotation = agent_state.rotation
    Rot = R.from_quat(np.array([rotation.x, rotation.y, rotation.z, rotation.w]))
    Rot_matrix = Rot.as_matrix()
    
    H = np.eye(4)
    H[:3, :3] = Rot_matrix
    H[:3, 3] = np.array([x, y, height])
    
    return H

def get_default_transform(agent_state):
    """ Get 4x4 transform from agent state (c2w) """
    rotation = agent_state.rotation
    Rot = R.from_quat(np.array([rotation.x, rotation.y, rotation.z, rotation.w]))
    Rot_matrix = Rot.as_matrix()
    ...
    H = np.eye(4)
    H[:3, :3] = Rot_matrix
    H[:3, 3] = np.array([agent_state.position[0], agent_state.position[1], agent_state.position[2]])
    
    return H

def get_cam_transform(agent_state):
    """ Get 4x4 transform from agent state (c2w) """
    ...
    H = np.eye(4)
    H[:3, :3] = Rot_matrix
    H[:3, 3] = np.array([sensor_state.position[0], sensor_state.position[1], sensor_state.position[2]])
...
def depth_to_3D(depth_obs, img_size, xs, ys, inv_K):
    ...
    depth = depth_obs.reshape(1, img_size[0], img_size[1])
...
def sample_free_particles(depth_obs, img_size, xs, ys, inv_K):
    """
    Sample Free Particles from depth

    Args:
        depth_obs: (H, W) tensor
        img_size: (H, W) tuple
        xs: (1, H, W) tensor
        ys: (1, H, W) tensor
        inv_K: (3, 3) tensor
    ...
    sampled_z = torch.rand((10, 1, 1), device=depth.device) * (depth - 0.05) # (K, H, W)
    sampled_z.clamp_(min=0.)

    # Unproject
    # negate depth as the camera looks along -Z
    # SPEEDUP - create ones in constructor
    xys = torch.stack((torch.mul(xs, sampled_z) , torch.mul(ys, sampled_z), -sampled_z, torch.ones(sampled_z.shape, device='cuda')), dim = 0) # 4 x K x 128 x 128
    xys = xys.reshape(4, -1)
    xy_c0 = torch.matmul(inv_K, xys)

    # SPEEDUP - don't allocate new memory, manipulate existing shapes
    free3D = torch.zeros((xy_c0.shape[1],3), dtype=torch.float32, device='cuda')
    free3D[:,0] = xy_c0[0,:]
    free3D[:,1] = xy_c0[1,:]
    free3D[:,2] = xy_c0[2,:]
