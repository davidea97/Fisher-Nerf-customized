...
from models.SLAM.utils.geometryutils import relative_transformation
from models.SLAM.utils.recon_helpers import setup_camera
from models.SLAM.utils.slam_external import build_rotation, calc_psnr
from models.SLAM.utils.slam_helpers import transform_to_frame, transformed_params2rendervar, transformed_params2depthplussilhouette
...
def report_progress(params, data, i, progress_bar, iter_time_idx, sil_thres, every_i=1, qual_every_i=1, 
    ...
    rmse_meter = AverageMeter('RMSE')
    l1_meter = AverageMeter('Depth L1')
    pose_error_meter = AverageMeter('Abs Pose Error')
    rel_pt_error_meter = AverageMeter('Rel Pose Error')
    ate_rmse_meter = AverageMeter('ATE RMSE')
    ...
    if i % every_i == 0 or i == 1:
        ...
        # tracking & there is GT pose data
        if tracking and data['iter_gt_w2c_list'][0] is not None:
            ...
            pose_error_meter.update(iter_pt_error)
            if iter_time_idx > 0:
        ...
        l1_meter.update(depth_l1.item())
        rmse_meter.update(rmse.item())

        if not (tracking or mapping):
            progress_bar.set_postfix({f"Time-Step: {iter_time_idx} | PSNR: {psnr_meter.avg:.{4}} | Depth RMSE: {rmse_meter.avg:.{4}} | L1": f"{l1_meter.avg:.{4}}"})
            progress_bar.update(every_i)
        elif tracking:
            progress_bar.set_postfix({f"Time-Step: {iter_time_idx} | Rel Pose Error: {rel_pt_error_meter.avg:.{4}} | Pose Error: {pose_error_meter.avg:.{4}} | ATE RMSE": f"{ate_rmse_meter.avg:.{4}}"})
            progress_bar.update(every_i)
        elif mapping:
            progress_bar.set_postfix({f"Time-Step: {online_time_idx} | Frame {data['id']} | PSNR: {psnr_meter.avg:.{4}} | Depth RMSE: {rmse_meter.avg :.{4}} | L1": f"{l1_meter.avg:.{4}}"})
            progress_bar.update(every_i)
