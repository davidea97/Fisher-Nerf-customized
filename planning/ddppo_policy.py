...
class DdppoPolicy(nn.Module):
    def __init__(self,
        ...
        if "ckpt.11.pth" in path:
            self.hidden_size = checkpoint['config'].RL.PPO.hidden_size
        else:
        ...
        self.hidden_state = torch.zeros(1, self.actor_critic.net.num_recurrent_layers, self.hidden_size)
    ...
    def plan(self, depth, goal,t):
        ...
        return_pkg = self.actor_critic.act(batch,
                                            self.hidden_state.to(depth.device),
                                            self.prev_actions.to(depth.device),
                                            not_done_masks,
                                            deterministic=False)
        actions, self.hidden_state = return_pkg.actions, return_pkg.rnn_hidden_states
