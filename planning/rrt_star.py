...
class BiRRT(RRT):
    ...
    def planning(self, animation=True, use_straight_line=False, exploration=False, horizon=10):
        ...
        animation: flag for animation on or off .
        """
        if np.any(self.occupancy_map[self.goal_node.x-3:self.goal_node.x+4, self.goal_node.y-3:self.goal_node.y+4]):
            return None
        
        goal_tree = [self.end]
        tree1, tree2  = self.node_list, goal_tree

        if self.check_node_collision_map(self.start, self.end, self.occupancy_map):
        ...
        for i in range(self.max_iter):
            ...
            ratio = len(self.node_list) / (len(self.node_list) + len(goal_tree))
            swap = random.uniform(0, 1) < ratio
            if swap:
                tree1, tree2 = tree2, tree1

            nearest_ind = self.get_nearest_node_index(tree1, rnd)
            new_node = self.steer(tree1[nearest_ind], rnd, self.expand_dis)
            near_node = tree1[nearest_ind]
            d, angle = self.calc_distance_and_angle(near_node, new_node)
            if d < 1:
                continue
            
            # if self.check_collision(new_node, self.obstacle_list):
            if self.check_collision_map(new_node, self.occupancy_map):
                self.node_list.append(near_node)

                nearest_ind2 = self.get_nearest_node_index(tree2, rnd)
                if self.check_node_collision_map(new_node, tree2[nearest_ind2], self.occupancy_map):
    ...
    def straight_line_to_goal(self, node):
        ...
        for i in range(len(xs)):
            x = int(xs[i])
            y = int(ys[i])
            if np.any(self.occupancy_map[x-3:x+4, y-3:y+4]):
                return False
    ...
    def search_best_goal_node(self, threshold=5):
        dist_to_goal_list = [
            self.calc_dist_to_goal(n.x, n.y) for n in self.node_list
        ]
        goal_inds = [
...
class Astar:
    def __init__(self, start, occupancy_map) -> None:
        """
    ...
    def planning(self, goal):
        # check goal first
        if self.occ_map[goal[0], goal[1]]:
            return None

        dist_to_goal_map = np.ones_like(self.occ_map, dtype=np.float32) * -1
        ...
        while True:
            ...
            inside = (neighbor_path[:, :, 0] >= 0) & (neighbor_path[:, :, 0] < self.occ_map.shape[0]) &  (neighbor_path[:, :, 1] >= 0) & (neighbor_path[:, :, 1] < self.occ_map.shape[1])
            inside = np.all(inside, axis=1)

            # remove the neighbors out of the map
            # inside = (neighbors[:, 0] >= 0) & (neighbors[:, 0] < self.occ_map.shape[0]) & (neighbors[:, 1] >= 0) & (neighbors[:, 1] < self.occ_map.shape[1])
            ...
            for neighbor, neighbor_path in zip(neighbors, neighbor_path):
                ...
                collision_cost_array[dist_obs > 4] = 0
                collision_cost_array[np.bitwise_and(dist_obs > 2, dist_obs <= 4)] = 2
                collision_cost_array[np.bitwise_and(dist_obs > 1, dist_obs <= 2)] = 4
                collision_cost_array[dist_obs <= 1] = 6

                # if distance_to_obstacle[neighbor[0], neighbor[1]] > 4:
                #     collision_risk = 0
                # elif distance_to_obstacle[neighbor[0], neighbor[1]] > 2:
                #     collision_risk = 2
                # elif distance_to_obstacle[neighbor[0], neighbor[1]] > 1:
                #     collision_risk = 4
                # else:
                #     collision_risk = 6
